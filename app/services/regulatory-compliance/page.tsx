...
const BENEFITS = [
  { icon: "📋", title: "Licensing Advisory", desc: "Guidance on obtaining and maintaining business licences across UAE mainland, free zones, and offshore jurisdictions." },
  { icon: "🔍", title: "Compliance Audits", desc: "Comprehensive regulatory compliance audits identifying gaps and remediation priorities." },
  { icon: "⚖️", title: "Enforcement Defence", desc: "Legal representation and advisory in regulatory investigations, enforcement actions, and sanctions proceedings." },
  { icon: "📄", title: "Regulatory Filings", desc: "Preparation and submission of all required regulatory filings, annual returns, and notifications." },
  { icon: "🏛️", title: "Authority Liaison", desc: "Direct liaison with UAE regulatory authorities including DED, CBUAE, SCA, and DIFC regulators." },
  { icon: "📘", title: "Compliance Programmes", desc: "Design and implementation of tailored corporate compliance frameworks and training programmes." },
];
const STEPS = [
  { num: "01", title: "Regulatory Mapping", desc: "We identify all applicable regulatory requirements for your industry, activities, and jurisdiction.", duration: "Week 1" },
  { num: "02", title: "Compliance Gap Analysis", desc: "We assess your current compliance posture against applicable regulations and identify gaps.", duration: "Week 1–2" },
  { num: "03", title: "Remediation Plan", desc: "We develop a prioritised remediation roadmap with clear timelines and responsibilities.", duration: "Week 2–3" },
  { num: "04", title: "Implementation Support", desc: "Our team supports implementation of policies, procedures, and controls to close compliance gaps.", duration: "Ongoing" },
  { num: "05", title: "Regulatory Filings", desc: "We prepare and submit all required filings, registrations, and renewals to relevant authorities.", duration: "As required" },
  { num: "06", title: "Ongoing Monitoring", desc: "We monitor regulatory changes and advise on required updates to your compliance programme.", duration: "Continuous" },
];
const FAQS = [
  { q: "What regulatory bodies does 111 Group work with?", a: "We work with all major UAE regulatory authorities including DED, DIFC, ADGM, CBUAE, SCA, MOHRE, and relevant free zone authorities." },
  { q: "Does 111 Group handle financial services regulation?", a: "Yes. We advise on CBUAE, SCA, and DIFC/ADGM financial services regulatory requirements, including licensing, compliance frameworks, and enforcement defence." },
  { q: "How long does a business licence application take in the UAE?", a: "UAE mainland licence applications typically take 1–4 weeks. Free zone licences can be obtained in as little as 1–5 business days. We manage the full process." },
  { q: "What happens if my business is subject to a regulatory investigation?", a: "We provide immediate defence advisory and representation in regulatory investigations, including document production, authority interviews, and mitigation strategy." },
  { q: "Can 111 Group help build an internal compliance programme?", a: "Yes. We design comprehensive compliance frameworks including policies, procedures, training, monitoring, and reporting structures tailored to your industry and regulatory environment." },
];
...
export default function RegulatoryCompliancePage() {
  ...
  return (
    <>
      <Modal overlayRef={modal.overlayRef} contentRef={modal.contentRef} close={modal.close} title="Regulatory Compliance & Enforcement" />
      <section className="relative min-h-[72vh] flex flex-col justify-end overflow-hidden">
        <div className="absolute inset-0"><Image src="/brand/images/expandable-cards/legal.jpg" alt="Regulatory Compliance" fill className="object-cover object-center" priority /><div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/50 to-black/88" /></div>
        <div className="relative mx-auto w-full max-w-[1440px] px-6 lg:px-10 pb-16 pt-40">
          <div className="flex items-center gap-2 text-[0.67rem] text-white/50 mb-6"><Link href="/" className="hover:text-white/80 transition-colors">Home</Link><span>/</span><Link href="/services" className="hover:text-white/80 transition-colors">Services</Link><span>/</span><span className="text-white/80">Regulatory Compliance & Enforcement</span></div>
          <div className="max-w-2xl"><div className="flex items-center gap-2 mb-4"><span className="text-[color:var(--color-brand)]">✦</span><span className="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-white/60">Legal Advisory</span></div>
            <h1 className="font-display text-[clamp(2.4rem,5.5vw,4.2rem)] font-semibold leading-[1.05] tracking-[-0.02em] text-white mb-5">Stay Compliant.<br/>Stay Protected.</h1>
            <p className="text-[1rem] text-white/65 leading-relaxed max-w-lg">End-to-end regulatory compliance consulting for businesses in the UAE — from licensing and audits to enforcement defence and ongoing regulatory monitoring.</p>
          </div>
        </div>
      </section>
      <div className="bg-bg"><div className="mx-auto max-w-[1440px] px-6 lg:px-10 py-14 lg:py-20"><div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_300px]">
        <div className="flex flex-col gap-16 min-w-0">
          <section id="benefits"><p className="eyebrow eyebrow-gold mb-2">What We Offer</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">Our Regulatory Services</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-8">Comprehensive regulatory support from licensing to enforcement defence.</p><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{BENEFITS.map((b) => (<div key={b.title} className="rounded-2xl bg-bg-elev p-6"><div className="text-2xl mb-3">{b.icon}</div><h3 className="font-semibold text-fg text-[0.88rem] mb-2">{b.title}</h3><p className="text-[0.76rem] text-fg-soft leading-relaxed">{b.desc}</p></div>))}</div></section>
          <section id="steps"><p className="eyebrow eyebrow-gold mb-2">Steps & Procedure</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">How the Process Works</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-10">From regulatory mapping to continuous compliance monitoring.</p><div className="flex flex-col gap-0">{STEPS.map((s, i) => (<div key={s.num} className="flex gap-5"><div className="flex flex-col items-center"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[color:var(--color-brand)] text-white text-[0.68rem] font-bold">{s.num}</div>{i < STEPS.length - 1 && <div className="w-px flex-1 bg-line my-2" />}</div><div className="pb-8 mt-1.5"><div className="flex items-center gap-3 mb-1.5"><h3 className="font-display text-[0.95rem] font-semibold text-fg">{s.title}</h3><span className="rounded-full bg-bg px-2.5 py-0.5 text-[0.58rem] font-medium text-fg-muted">{s.duration}</span></div><p className="text-[0.8rem] text-fg-soft leading-relaxed">{s.desc}</p></div></div>))}</div></section>
