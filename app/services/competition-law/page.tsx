...
const BENEFITS = [
  { icon: "🔀", title: "Merger Clearance", desc: "Pre-merger notifications, competition authority filings, and clearance strategy across GCC and international jurisdictions." },
  { icon: "🔍", title: "Cartel Investigations", desc: "Defence and representation in cartel investigations by the UAE Ministry of Economy and regional authorities." },
  { icon: "📊", title: "Dominance & Abuse", desc: "Advisory on dominant market position, abuse of dominance, and exclusionary practices compliance." },
  { icon: "📋", title: "Regulatory Filings", desc: "Preparation and submission of all required competition law filings and notifications." },
  { icon: "🏛️", title: "Regulatory Liaison", desc: "Direct liaison with competition authorities including the UAE Ministry of Economy and CBUAE." },
  { icon: "📘", title: "Compliance Programmes", desc: "Design and implementation of internal competition law compliance programmes for corporates." },
];
const STEPS = [
  { num: "01", title: "Matter Assessment", desc: "We assess the competition law implications of your transaction, conduct, or investigation exposure.", duration: "Day 1–3" },
  { num: "02", title: "Legal Analysis", desc: "Our competition law specialists analyse applicable laws, market definitions, and regulatory thresholds.", duration: "Week 1" },
  { num: "03", title: "Strategy Development", desc: "We develop a tailored strategy for clearance, defence, or compliance depending on your matter.", duration: "Week 1–2" },
  { num: "04", title: "Authority Filings", desc: "We prepare and submit all required notifications, responses, and supporting economic analysis.", duration: "Week 2–4" },
  { num: "05", title: "Regulatory Proceedings", desc: "We represent you at hearings and in all correspondence with competition authorities.", duration: "Ongoing" },
  { num: "06", title: "Resolution & Compliance", desc: "We assist in implementing any remedies, commitments, or compliance measures required.", duration: "Final stage" },
];
const FAQS = [
  { q: "What is competition law in the UAE?", a: "The UAE Federal Law No. 4 of 2012 (as amended) regulates anti-competitive practices, merger control, and abuse of dominant position in the UAE market." },
  { q: "When is merger notification required in the UAE?", a: "Notification is required when a transaction meets the UAE competition law thresholds relating to market share and turnover. 111 Group will assess your specific transaction." },
  { q: "What are the penalties for competition law violations?", a: "Penalties can include substantial fines of up to AED 5 million, transaction prohibitions, and in serious cases, criminal liability for individuals." },
  { q: "Does 111 Group handle competition investigations?", a: "Yes. We provide full defence services in competition authority investigations, including dawn raids, information requests, and enforcement proceedings." },
  { q: "Can 111 Group help design a compliance programme?", a: "Absolutely. We design and implement tailored competition law compliance programmes, including training, policy drafting, and ongoing monitoring." },
];
...
export default function CompetitionLawPage() {
  ...
  return (
    <>
      <Modal overlayRef={modal.overlayRef} contentRef={modal.contentRef} close={modal.close} title="Competition Law & Anti-Trust" />
      <section className="relative min-h-[72vh] flex flex-col justify-end overflow-hidden">
        <div className="absolute inset-0"><Image src="/brand/images/expandable-cards/legal.jpg" alt="Competition Law" fill className="object-cover object-center" priority /><div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/50 to-black/88" /></div>
        <div className="relative mx-auto w-full max-w-[1440px] px-6 lg:px-10 pb-16 pt-40">
          <div className="flex items-center gap-2 text-[0.67rem] text-white/50 mb-6"><Link href="/" className="hover:text-white/80 transition-colors">Home</Link><span>/</span><Link href="/services" className="hover:text-white/80 transition-colors">Services</Link><span>/</span><span className="text-white/80">Competition Law & Anti-Trust</span></div>
          <div className="max-w-2xl"><div className="flex items-center gap-2 mb-4"><span className="text-[color:var(--color-brand)]">✦</span><span className="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-white/60">Legal Advisory</span></div>
            <h1 className="font-display text-[clamp(2.4rem,5.5vw,4.2rem)] font-semibold leading-[1.05] tracking-[-0.02em] text-white mb-5">Competition Law<br/>& Anti-Trust Advisory.</h1>
            <p className="text-[1rem] text-white/65 leading-relaxed max-w-lg">Advisory and representation on competition law compliance, merger filings, investigations, and anti-trust litigation across GCC and international jurisdictions.</p>
          </div>
        </div>
      </section>
      <div className="bg-bg"><div className="mx-auto max-w-[1440px] px-6 lg:px-10 py-14 lg:py-20"><div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_300px]">
        <div className="flex flex-col gap-16 min-w-0">
          <section id="benefits"><p className="eyebrow eyebrow-gold mb-2">What We Offer</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">Our Competition Law Services</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-8">Comprehensive advisory covering the full spectrum of competition law matters.</p><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{BENEFITS.map((b) => (<div key={b.title} className="rounded-2xl bg-bg-elev p-6"><div className="text-2xl mb-3">{b.icon}</div><h3 className="font-semibold text-fg text-[0.88rem] mb-2">{b.title}</h3><p className="text-[0.76rem] text-fg-soft leading-relaxed">{b.desc}</p></div>))}</div></section>
          <section id="steps"><p className="eyebrow eyebrow-gold mb-2">Steps & Procedure</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">How the Process Works</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-10">From matter assessment to authority clearance — we guide you through every step.</p><div className="flex flex-col gap-0">{STEPS.map((s, i) => (<div key={s.num} className="flex gap-5"><div className="flex flex-col items-center"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[color:var(--color-brand)] text-white text-[0.68rem] font-bold">{s.num}</div>{i < STEPS.length - 1 && <div className="w-px flex-1 bg-line my-2" />}</div><div className="pb-8 mt-1.5"><div className="flex items-center gap-3 mb-1.5"><h3 className="font-display text-[0.95rem] font-semibold text-fg">{s.title}</h3><span className="rounded-full bg-bg px-2.5 py-0.5 text-[0.58rem] font-medium text-fg-muted">{s.duration}</span></div><p className="text-[0.8rem] text-fg-soft leading-relaxed">{s.desc}</p></div></div>))}</div></section>
