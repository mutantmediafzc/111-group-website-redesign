...
const BENEFITS = [
  { icon: "🏠", title: "Property Acquisition", desc: "Full legal advisory on residential and commercial property purchases in UAE mainland and free zones." },
  { icon: "📋", title: "Contract Review", desc: "Review and negotiation of SPA, MOU, and developer agreements to protect your interests." },
  { icon: "🔍", title: "Title Due Diligence", desc: "Comprehensive title searches, encumbrance checks, and property ownership verification." },
  { icon: "🏢", title: "Off-Plan Advisory", desc: "Advisory on off-plan purchases, escrow compliance, and developer RERA registration checks." },
  { icon: "💰", title: "Investment Structuring", desc: "Optimal ownership structures for real estate investors — direct ownership, SPV, or trust arrangements." },
  { icon: "⚖️", title: "Property Disputes", desc: "Representation in RERA disputes, court proceedings, and developer-buyer conflict resolution." },
];
const STEPS = [
  { num: "01", title: "Initial Consultation", desc: "We review your investment objectives, budget, and property type to advise on the optimal acquisition structure.", duration: "Day 1" },
  { num: "02", title: "Property Due Diligence", desc: "We conduct full title searches, encumbrance checks, and developer status verification.", duration: "Week 1" },
  { num: "03", title: "Contract Review", desc: "Our lawyers review and negotiate the SPA or MOU to ensure all terms protect your interests.", duration: "Week 1–2" },
  { num: "04", title: "Transaction Support", desc: "We manage the full transaction process including NOC applications, transfer coordination, and DLD registration.", duration: "Week 2–4" },
  { num: "05", title: "Title Registration", desc: "We coordinate registration of your title deed at the Dubai Land Department or relevant authority.", duration: "Week 4–6" },
  { num: "06", title: "Post-Completion", desc: "We advise on property management, tenancy agreements, and any post-completion obligations.", duration: "Final stage" },
];
const FAQS = [
  { q: "Can foreigners buy property in Dubai?", a: "Yes. Foreign nationals can purchase freehold property in designated areas of Dubai. We advise on eligible zones and manage the full acquisition process." },
  { q: "What is the property transfer fee in Dubai?", a: "The Dubai Land Department charges a 4% transfer fee on the property value. Additional fees apply for registration, mortgage, and NOC applications. We provide a full cost breakdown." },
  { q: "What legal checks should I do before buying property in the UAE?", a: "Key checks include title deed verification, encumbrance search, developer RERA registration, escrow account status, and service charge liability confirmation. We manage all of these." },
  { q: "Can 111 Group help with off-plan property purchases?", a: "Yes. We advise on all aspects of off-plan purchases, including developer vetting, escrow account verification, SPA review, and RERA registration compliance." },
  { q: "Does 111 Group handle property disputes?", a: "Yes. We represent buyers, sellers, and landlords in RERA disputes, rental disputes, court proceedings, and mediation for all types of real estate conflicts." },
];
...
export default function RealEstateAdvisoryPage() {
  ...
  return (
    <>
      <Modal overlayRef={modal.overlayRef} contentRef={modal.contentRef} close={modal.close} title="Real Estate Advisory" />
      <section className="relative min-h-[72vh] flex flex-col justify-end overflow-hidden">
        <div className="absolute inset-0"><Image src="/brand/images/expandable-cards/legal.jpg" alt="Real Estate Advisory" fill className="object-cover object-center" priority /><div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/50 to-black/88" /></div>
        <div className="relative mx-auto w-full max-w-[1440px] px-6 lg:px-10 pb-16 pt-40">
          <div className="flex items-center gap-2 text-[0.67rem] text-white/50 mb-6"><Link href="/" className="hover:text-white/80 transition-colors">Home</Link><span>/</span><Link href="/services" className="hover:text-white/80 transition-colors">Services</Link><span>/</span><span className="text-white/80">Real Estate Advisory</span></div>
          <div className="max-w-2xl"><div className="flex items-center gap-2 mb-4"><span className="text-[color:var(--color-brand)]">✦</span><span className="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-white/60">Real Estate & Wealth</span></div>
            <h1 className="font-display text-[clamp(2.4rem,5.5vw,4.2rem)] font-semibold leading-[1.05] tracking-[-0.02em] text-white mb-5">Invest in UAE Real Estate<br/>With Confidence.</h1>
            <p className="text-[1rem] text-white/65 leading-relaxed max-w-lg">Legal advisory and transaction support for property purchases, investment structuring, and real estate dispute resolution across UAE mainland and free zones.</p>
          </div>
        </div>
      </section>
      <div className="bg-bg"><div className="mx-auto max-w-[1440px] px-6 lg:px-10 py-14 lg:py-20"><div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_300px]">
        <div className="flex flex-col gap-16 min-w-0">
          <section id="benefits"><p className="eyebrow eyebrow-gold mb-2">What We Offer</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">Our Real Estate Services</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-8">From due diligence to title registration — complete real estate legal support.</p><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{BENEFITS.map((b) => (<div key={b.title} className="rounded-2xl bg-bg-elev p-6"><div className="text-2xl mb-3">{b.icon}</div><h3 className="font-semibold text-fg text-[0.88rem] mb-2">{b.title}</h3><p className="text-[0.76rem] text-fg-soft leading-relaxed">{b.desc}</p></div>))}</div></section>
          <section id="steps"><p className="eyebrow eyebrow-gold mb-2">Steps & Procedure</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">How the Process Works</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-10">From initial consultation to title deed in hand — we manage every step.</p><div className="flex flex-col gap-0">{STEPS.map((s, i) => (<div key={s.num} className="flex gap-5"><div className="flex flex-col items-center"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[color:var(--color-brand)] text-white text-[0.68rem] font-bold">{s.num}</div>{i < STEPS.length - 1 && <div className="w-px flex-1 bg-line my-2" />}</div><div className="pb-8 mt-1.5"><div className="flex items-center gap-3 mb-1.5"><h3 className="font-display text-[0.95rem] font-semibold text-fg">{s.title}</h3><span className="rounded-full bg-bg px-2.5 py-0.5 text-[0.58rem] font-medium text-fg-muted">{s.duration}</span></div><p className="text-[0.8rem] text-fg-soft leading-relaxed">{s.desc}</p></div></div>))}</div></section>
