...
const BENEFITS = [
  { icon: "🚢", title: "Vessel Registration", desc: "UAE and international vessel registration, flag state advisory, and maritime authority liaison." },
  { icon: "📦", title: "Cargo Disputes", desc: "Legal representation in cargo damage, loss, and delay claims under bills of lading and charterparties." },
  { icon: "📋", title: "Charter Party Review", desc: "Drafting, reviewing, and negotiating time charter, voyage charter, and bareboat charter agreements." },
  { icon: "🔱", title: "P&I Club Advisory", desc: "Liaison with P&I clubs and hull & machinery underwriters for claims and coverage advisory." },
  { icon: "⚓", title: "Port State Control", desc: "Representation and advisory in port state control inspections, detentions, and enforcement proceedings." },
  { icon: "🌊", title: "Salvage & Collision", desc: "Legal advisory and representation in maritime salvage, collision, and general average matters." },
];
const STEPS = [
  { num: "01", title: "Matter Assessment", desc: "We review your maritime matter, relevant contracts, and applicable law to advise on the best course of action.", duration: "Day 1–2" },
  { num: "02", title: "Evidence Preservation", desc: "We advise on immediate evidence preservation measures and, where required, appoint surveyors.", duration: "Day 1–5" },
  { num: "03", title: "Demand & Negotiation", desc: "We issue formal demands and conduct pre-litigation negotiations to achieve early settlement where possible.", duration: "Week 1–4" },
  { num: "04", title: "Arrest Proceedings", desc: "Where necessary, we obtain vessel or cargo arrest orders through UAE courts to secure your claim.", duration: "Week 2–4" },
  { num: "05", title: "Arbitration / Litigation", desc: "We manage arbitration or court proceedings from filing through to judgment or award.", duration: "Variable" },
  { num: "06", title: "Enforcement", desc: "We enforce judgments and awards against vessels, cargo, and other maritime assets.", duration: "Final stage" },
];
const FAQS = [
  { q: "What maritime law services does 111 Group provide?", a: "We cover vessel registration, cargo claims, charterparty disputes, P&I club liaison, port state control, salvage, collision, and maritime arbitration." },
  { q: "Can 111 Group obtain vessel arrests in the UAE?", a: "Yes. We have extensive experience obtaining vessel arrests before UAE courts as security for maritime claims, including arrest applications on an urgent basis." },
  { q: "Which maritime arbitration institutions does 111 Group work with?", a: "We work with LMAA (London Maritime Arbitrators Association), DIFC-LCIA, and ad hoc maritime arbitration proceedings under UNCITRAL rules." },
  { q: "Does 111 Group handle cargo insurance claims?", a: "Yes. We advise on cargo insurance claims, work with P&I clubs, and liaise with hull and cargo underwriters to secure prompt claim resolution." },
  { q: "Can 111 Group assist with vessel registration in the UAE?", a: "Yes. We manage vessel registration with the UAE Federal Transport Authority and advise on flag state selection, registration requirements, and ongoing compliance." },
];
...
export default function ShippingMaritimePage() {
  ...
  return (
    <>
      <Modal overlayRef={modal.overlayRef} contentRef={modal.contentRef} close={modal.close} title="Shipping & Maritime Law" />
      <section className="relative min-h-[72vh] flex flex-col justify-end overflow-hidden">
        <div className="absolute inset-0"><Image src="/brand/images/expandable-cards/legal.jpg" alt="Shipping & Maritime" fill className="object-cover object-center" priority /><div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/50 to-black/88" /></div>
        <div className="relative mx-auto w-full max-w-[1440px] px-6 lg:px-10 pb-16 pt-40">
          <div className="flex items-center gap-2 text-[0.67rem] text-white/50 mb-6"><Link href="/" className="hover:text-white/80 transition-colors">Home</Link><span>/</span><Link href="/services" className="hover:text-white/80 transition-colors">Services</Link><span>/</span><span className="text-white/80">Shipping & Maritime Law</span></div>
          <div className="max-w-2xl"><div className="flex items-center gap-2 mb-4"><span className="text-[color:var(--color-brand)]">✦</span><span className="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-white/60">Legal Advisory</span></div>
            <h1 className="font-display text-[clamp(2.4rem,5.5vw,4.2rem)] font-semibold leading-[1.05] tracking-[-0.02em] text-white mb-5">Shipping & Maritime<br/>Legal Expertise.</h1>
            <p className="text-[1rem] text-white/65 leading-relaxed max-w-lg">Specialised legal services for the maritime and shipping sector — vessel registration, cargo disputes, charter agreements, and port state control across UAE and international waters.</p>
          </div>
        </div>
      </section>
      <div className="bg-bg"><div className="mx-auto max-w-[1440px] px-6 lg:px-10 py-14 lg:py-20"><div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_300px]">
        <div className="flex flex-col gap-16 min-w-0">
          <section id="benefits"><p className="eyebrow eyebrow-gold mb-2">What We Offer</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">Our Maritime Services</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-8">Full-service maritime law practice covering vessels, cargo, charters, and regulatory matters.</p><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{BENEFITS.map((b) => (<div key={b.title} className="rounded-2xl bg-bg-elev p-6"><div className="text-2xl mb-3">{b.icon}</div><h3 className="font-semibold text-fg text-[0.88rem] mb-2">{b.title}</h3><p className="text-[0.76rem] text-fg-soft leading-relaxed">{b.desc}</p></div>))}</div></section>
          <section id="steps"><p className="eyebrow eyebrow-gold mb-2">Steps & Procedure</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">How the Process Works</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-10">From immediate preservation to enforcement — expert maritime case management.</p><div className="flex flex-col gap-0">{STEPS.map((s, i) => (<div key={s.num} className="flex gap-5"><div className="flex flex-col items-center"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[color:var(--color-brand)] text-white text-[0.68rem] font-bold">{s.num}</div>{i < STEPS.length - 1 && <div className="w-px flex-1 bg-line my-2" />}</div><div className="pb-8 mt-1.5"><div className="flex items-center gap-3 mb-1.5"><h3 className="font-display text-[0.95rem] font-semibold text-fg">{s.title}</h3><span className="rounded-full bg-bg px-2.5 py-0.5 text-[0.58rem] font-medium text-fg-muted">{s.duration}</span></div><p className="text-[0.8rem] text-fg-soft leading-relaxed">{s.desc}</p></div></div>))}</div></section>
