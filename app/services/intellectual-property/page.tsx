...
const BENEFITS = [
  { icon: "™️", title: "Trademark Registration", desc: "UAE and international trademark registration across GCC, EU, and WIPO member countries." },
  { icon: "💡", title: "Patent Filing", desc: "Patent applications and prosecution in the UAE, GCC, and through international patent treaties." },
  { icon: "©️", title: "Copyright Protection", desc: "Registration and enforcement of copyrights for creative works, software, and digital content." },
  { icon: "⚖️", title: "IP Enforcement", desc: "Cease and desist actions, customs recordals, and court proceedings against IP infringers." },
  { icon: "📄", title: "Licensing Agreements", desc: "Drafting, reviewing, and negotiating IP licensing, franchise, and technology transfer agreements." },
  { icon: "🔐", title: "Trade Secrets", desc: "Advisory on trade secret protection, confidentiality agreements, and enforcement in the UAE." },
];
const STEPS = [
  { num: "01", title: "IP Audit", desc: "We assess your existing IP portfolio and identify gaps in protection and registration.", duration: "Week 1" },
  { num: "02", title: "Search & Clearance", desc: "We conduct comprehensive trademark and patent searches to assess registrability and conflicts.", duration: "Week 1–2" },
  { num: "03", title: "Application Filing", desc: "We prepare and file trademark, patent, or copyright applications with the relevant IP offices.", duration: "Week 2–3" },
  { num: "04", title: "Prosecution", desc: "We respond to office actions, objections, and opposition proceedings on your behalf.", duration: "Ongoing" },
  { num: "05", title: "Registration & Certificate", desc: "Upon approval, we secure your IP registration certificate and advise on maintenance.", duration: "Variable" },
  { num: "06", title: "Enforcement & Monitoring", desc: "We monitor for infringement and take appropriate enforcement action to protect your rights.", duration: "Continuous" },
];
const FAQS = [
  { q: "How long does trademark registration take in the UAE?", a: "UAE trademark registration typically takes 6–12 months, depending on whether objections or oppositions are raised. We manage the full prosecution process." },
  { q: "Can 111 Group register trademarks internationally?", a: "Yes. We manage international trademark registrations through the Madrid Protocol (WIPO), GCC collective filing, and direct national filings across key markets." },
  { q: "What is the duration of UAE trademark protection?", a: "A UAE trademark registration is valid for 10 years from the application filing date and is renewable indefinitely." },
  { q: "What happens if someone infringes my trademark in the UAE?", a: "We can initiate enforcement action including cease and desist letters, customs border measures, and civil litigation in UAE courts or DIFC." },
  { q: "Does 111 Group handle patent applications?", a: "Yes. We handle patent applications through the UAE Patent Office, GCC Patent Office, and international patent systems. Our team includes specialist IP legal advisors." },
];
...
export default function IntellectualPropertyPage() {
  ...
  return (
    <>
      <Modal overlayRef={modal.overlayRef} contentRef={modal.contentRef} close={modal.close} title="Intellectual Property" />
      <section className="relative min-h-[72vh] flex flex-col justify-end overflow-hidden">
        <div className="absolute inset-0"><Image src="/brand/images/expandable-cards/legal.jpg" alt="Intellectual Property" fill className="object-cover object-center" priority /><div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/50 to-black/88" /></div>
        <div className="relative mx-auto w-full max-w-[1440px] px-6 lg:px-10 pb-16 pt-40">
          <div className="flex items-center gap-2 text-[0.67rem] text-white/50 mb-6"><Link href="/" className="hover:text-white/80 transition-colors">Home</Link><span>/</span><Link href="/services" className="hover:text-white/80 transition-colors">Services</Link><span>/</span><span className="text-white/80">Intellectual Property</span></div>
          <div className="max-w-2xl"><div className="flex items-center gap-2 mb-4"><span className="text-[color:var(--color-brand)]">✦</span><span className="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-white/60">Legal Advisory</span></div>
            <h1 className="font-display text-[clamp(2.4rem,5.5vw,4.2rem)] font-semibold leading-[1.05] tracking-[-0.02em] text-white mb-5">Protect Your Ideas.<br/>Secure Your Brand.</h1>
            <p className="text-[1rem] text-white/65 leading-relaxed max-w-lg">Registration, protection, and enforcement of trademarks, patents, copyrights, and trade secrets across UAE and international markets.</p>
          </div>
        </div>
      </section>
      <div className="bg-bg"><div className="mx-auto max-w-[1440px] px-6 lg:px-10 py-14 lg:py-20"><div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_300px]">
        <div className="flex flex-col gap-16 min-w-0">
          <section id="benefits"><p className="eyebrow eyebrow-gold mb-2">What We Offer</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">Our IP Services</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-8">Comprehensive intellectual property protection from registration to enforcement.</p><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{BENEFITS.map((b) => (<div key={b.title} className="rounded-2xl bg-bg-elev p-6"><div className="text-2xl mb-3">{b.icon}</div><h3 className="font-semibold text-fg text-[0.88rem] mb-2">{b.title}</h3><p className="text-[0.76rem] text-fg-soft leading-relaxed">{b.desc}</p></div>))}</div></section>
          <section id="steps"><p className="eyebrow eyebrow-gold mb-2">Steps & Procedure</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">How the Process Works</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-10">From IP audit to registration certificate — we manage your full IP lifecycle.</p><div className="flex flex-col gap-0">{STEPS.map((s, i) => (<div key={s.num} className="flex gap-5"><div className="flex flex-col items-center"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[color:var(--color-brand)] text-white text-[0.68rem] font-bold">{s.num}</div>{i < STEPS.length - 1 && <div className="w-px flex-1 bg-line my-2" />}</div><div className="pb-8 mt-1.5"><div className="flex items-center gap-3 mb-1.5"><h3 className="font-display text-[0.95rem] font-semibold text-fg">{s.title}</h3><span className="rounded-full bg-bg px-2.5 py-0.5 text-[0.58rem] font-medium text-fg-muted">{s.duration}</span></div><p className="text-[0.8rem] text-fg-soft leading-relaxed">{s.desc}</p></div></div>))}</div></section>
