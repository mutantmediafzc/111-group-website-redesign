...
const BENEFITS = [
  { icon: "📋", title: "PDPL Compliance", desc: "Advisory and implementation support for UAE Federal Decree-Law No. 45 of 2021 (Personal Data Protection Law)." },
  { icon: "🌍", title: "GDPR Advisory", desc: "Compliance assessment and gap analysis for businesses subject to EU GDPR requirements." },
  { icon: "🔍", title: "Data Audits", desc: "Comprehensive data mapping, processing activity audits, and risk assessments." },
  { icon: "📄", title: "Privacy Policies", desc: "Drafting and updating privacy notices, cookie policies, and data processing agreements." },
  { icon: "🚨", title: "Data Breach Response", desc: "Incident response planning, breach notification handling, and regulatory liaison." },
  { icon: "👤", title: "DPO Services", desc: "Outsourced Data Protection Officer services for organisations requiring a dedicated DPO under applicable law." },
];
const STEPS = [
  { num: "01", title: "Compliance Assessment", desc: "We assess your current data processing activities against PDPL and applicable privacy law requirements.", duration: "Week 1–2" },
  { num: "02", title: "Gap Analysis", desc: "We identify compliance gaps and prioritise remediation actions with a clear implementation roadmap.", duration: "Week 2–3" },
  { num: "03", title: "Policy Drafting", desc: "We draft or update all required privacy policies, notices, DPAs, and consent mechanisms.", duration: "Week 3–5" },
  { num: "04", title: "Implementation", desc: "We support implementation of technical and organisational measures to achieve compliance.", duration: "Ongoing" },
  { num: "05", title: "Training", desc: "We deliver staff training on data protection obligations, data handling procedures, and breach response.", duration: "As required" },
  { num: "06", title: "Ongoing Advisory", desc: "We provide continuous advisory on regulatory developments, audits, and breach incidents.", duration: "Continuous" },
];
const FAQS = [
  { q: "Does the UAE PDPL apply to my business?", a: "The UAE PDPL applies to personal data processing activities in the UAE and, in some cases, to businesses outside the UAE that process UAE residents' data." },
  { q: "What are the penalties for PDPL non-compliance?", a: "Penalties under the UAE PDPL can reach AED 20 million for serious violations. Our team helps you achieve full compliance to avoid regulatory exposure." },
  { q: "Do I need a Data Protection Officer?", a: "Under UAE PDPL, certain organisations must appoint a DPO. We offer outsourced DPO services for businesses that require a qualified DPO without hiring full-time." },
  { q: "How long does PDPL compliance take?", a: "A basic compliance programme typically takes 6–12 weeks. Larger organisations with complex data flows may require longer implementation timelines." },
  { q: "Can 111 Group handle data breach notifications?", a: "Yes. We provide 24/7 data breach response support, including regulatory notification, affected individual communication, and remediation guidance." },
];
...
export default function DataProtectionPage() {
  ...
  return (
    <>
      <Modal overlayRef={modal.overlayRef} contentRef={modal.contentRef} close={modal.close} title="Data Protection & Privacy Law" />
      <section className="relative min-h-[72vh] flex flex-col justify-end overflow-hidden">
        <div className="absolute inset-0"><Image src="/brand/images/expandable-cards/legal.jpg" alt="Data Protection" fill className="object-cover object-center" priority /><div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/50 to-black/88" /></div>
        <div className="relative mx-auto w-full max-w-[1440px] px-6 lg:px-10 pb-16 pt-40">
          <div className="flex items-center gap-2 text-[0.67rem] text-white/50 mb-6"><Link href="/" className="hover:text-white/80 transition-colors">Home</Link><span>/</span><Link href="/services" className="hover:text-white/80 transition-colors">Services</Link><span>/</span><span className="text-white/80">Data Protection & Privacy Law</span></div>
          <div className="max-w-2xl"><div className="flex items-center gap-2 mb-4"><span className="text-[color:var(--color-brand)]">✦</span><span className="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-white/60">Legal Advisory</span></div>
            <h1 className="font-display text-[clamp(2.4rem,5.5vw,4.2rem)] font-semibold leading-[1.05] tracking-[-0.02em] text-white mb-5">Data Protection<br/>& Privacy Compliance.</h1>
            <p className="text-[1rem] text-white/65 leading-relaxed max-w-lg">Advisory on UAE PDPL, GDPR compliance, privacy policies, data breach response, and outsourced DPO services for businesses operating in the UAE and globally.</p>
          </div>
        </div>
      </section>
      <div className="bg-bg"><div className="mx-auto max-w-[1440px] px-6 lg:px-10 py-14 lg:py-20"><div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_300px]">
        <div className="flex flex-col gap-16 min-w-0">
          <section id="benefits"><p className="eyebrow eyebrow-gold mb-2">What We Offer</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">Our Data Protection Services</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-8">End-to-end privacy compliance from gap analysis to ongoing DPO advisory.</p><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{BENEFITS.map((b) => (<div key={b.title} className="rounded-2xl bg-bg-elev p-6"><div className="text-2xl mb-3">{b.icon}</div><h3 className="font-semibold text-fg text-[0.88rem] mb-2">{b.title}</h3><p className="text-[0.76rem] text-fg-soft leading-relaxed">{b.desc}</p></div>))}</div></section>
          <section id="steps"><p className="eyebrow eyebrow-gold mb-2">Steps & Procedure</p><h2 className="font-display text-[clamp(1.6rem,3vw,2.4rem)] font-semibold leading-tight tracking-[-0.02em] text-fg mb-3">How the Process Works</h2><p className="text-[0.88rem] text-fg-soft max-w-xl leading-relaxed mb-10">From compliance assessment to ongoing regulatory support — a structured approach.</p><div className="flex flex-col gap-0">{STEPS.map((s, i) => (<div key={s.num} className="flex gap-5"><div className="flex flex-col items-center"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[color:var(--color-brand)] text-white text-[0.68rem] font-bold">{s.num}</div>{i < STEPS.length - 1 && <div className="w-px flex-1 bg-line my-2" />}</div><div className="pb-8 mt-1.5"><div className="flex items-center gap-3 mb-1.5"><h3 className="font-display text-[0.95rem] font-semibold text-fg">{s.title}</h3><span className="rounded-full bg-bg px-2.5 py-0.5 text-[0.58rem] font-medium text-fg-muted">{s.duration}</span></div><p className="text-[0.8rem] text-fg-soft leading-relaxed">{s.desc}</p></div></div>))}</div></section>
